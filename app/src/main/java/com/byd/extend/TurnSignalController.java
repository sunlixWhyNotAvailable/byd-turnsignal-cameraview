...
final class TurnSignalController {
    ...
    private void checkHealth() {
        ...
        if (LegacySettingsImporter.blocksRuntime(context)) {
            emit("runtime_blocked", "reason", "legacy_handover");
            CameraHelperService.pauseActiveRuntime();
            return;
        }
    ...
    private void ensureRunning(
        ...
        synchronized (LAUNCH_LOCK) {
            ...
            if (!migrateHelperNamespace()) {
                launchFailed("helper_namespace_migration_failed");
                return;
            }
    ...
    private boolean migrateHelperNamespace() {
        if (stopped || LegacySettingsImporter.blocksRuntime(context)) return false;
        return migrateHelperNamespace(settings, () -> {
            LocalAdbClient.Result result = LocalAdbClient.executeAuthorized(
                    context, helperNamespaceMigrationCommand(), this::emit);
            emit("helper_namespace_migration", "ok", result.ok, "error", result.error);
            return result;
        });
    }

    static boolean migrateHelperNamespace(
            SharedPreferences settings, Supplier<LocalAdbClient.Result> migration) {
        if (settings.getBoolean(KEY_HELPER_NAMESPACE_MIGRATED, false)) return true;
        LocalAdbClient.Result result = migration.get();
        return result.ok && settings.edit()
                .putBoolean(KEY_HELPER_NAMESPACE_MIGRATED, true).commit();
    }

    static String helperNamespaceMigrationCommand() {
        String previous = "/data/local/tmp/bydturnguard_awake_session";
        String current = "/data/local/tmp/bydextend_awake_session";
        return "(" + LegacySettingsImporter.legacyHelperCleanupCommand() + ") && "
                + "(if [ ! -e " + current + " ] && [ -f " + previous + " ]; then "
                + "cp " + previous + " " + current + ".migrating && mv "
                + current + ".migrating " + current + "; fi)";
    }
