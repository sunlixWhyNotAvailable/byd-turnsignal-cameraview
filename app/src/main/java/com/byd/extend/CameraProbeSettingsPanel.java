...
final class CameraProbeSettingsPanel {
    ...
    private final Button restoreLegacyButton;
    ...
    private boolean legacyRestoreAvailable;
    ...
    CameraProbeSettingsPanel(
        ...
        this(activity, preferences, shareLogsAction, compatibilityBundleAction,
                exportPresetAction, importPresetAction, importLegacyAction, null);
    }
    ...
    CameraProbeSettingsPanel(
            ...
            Runnable importPresetAction,
            Runnable importLegacyAction,
            Runnable restoreLegacyAccessAction) {
        ...
        restoreLegacyButton = activity.button("Відновити доступ до старого застосунку");
        legacyRestoreAvailable = restoreLegacyAccessAction != null;
        restoreLegacyButton.setVisibility(legacyRestoreAvailable ? View.VISIBLE : View.GONE);
        if (restoreLegacyAccessAction != null) {
            restoreLegacyButton.setOnClickListener(view -> restoreLegacyAccessAction.run());
        }
        root.addView(restoreLegacyButton, new LinearLayout.LayoutParams(
                LinearLayout.LayoutParams.MATCH_PARENT, activity.dp(52)));
        transferStatus = activity.label("Файл пресету містить вигляд камер без числових порогів. "
                + "Повний імпорт із 0.52.1 потребує ADB і зупиняє старий застосунок, "
                + "залишаючи його встановленим та доступним через значок.");
    ...
    private void updateExportButtons() {
        ...
        restoreLegacyButton.setEnabled(legacyRestoreAvailable && transferAllowed && !anyExport);
    }
    ...
    void setLegacyRestoreAvailable(boolean available) {
        legacyRestoreAvailable = available;
        restoreLegacyButton.setVisibility(available ? View.VISIBLE : View.GONE);
        updateExportButtons();
    }
