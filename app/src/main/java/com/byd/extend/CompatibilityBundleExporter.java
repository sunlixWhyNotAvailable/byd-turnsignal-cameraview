...
final class CompatibilityBundleExporter {
    ...
    static File export(Context context, SharedPreferences preferences) throws IOException {
        return export(context, preferences, new ExportControl());
    }
    ...
    static File export(
        ...
        return export(cacheDirectory, preferences, identity, textRunner, streamRunner,
                createdAtMillis, new ExportControl());
    }
    ...
    static File export(
        ...
        try {
            for (int textIndex = 0; textIndex < textSpecs.size(); textIndex++) {
                ...
                checkCancelled(control);
                progress.report(Progress.Phase.TEXT, spec.entry, textIndex + 1,
                        textSpecs.size(), 0L, textTotal, true);
                ...
                progress.report(Progress.Phase.TEXT, spec.entry, textIndex + 1,
                        textSpecs.size(), value.length, textTotal, false);
            }
        } catch (Throwable error) {
            if (partial.exists()) partial.delete();
            deleteTree(tempDirectory);
            if (error instanceof CancellationException || control.isCancellationRequested()) {
                progress.cancelled();
                if (error instanceof CancellationException) throw (CancellationException) error;
                throw new CancellationException();
            }
            if (error instanceof IOException) throw (IOException) error;
            throw new IOException("Unable to collect compatibility text", error);
        }
        ...
        try (ZipOutputStream zip = new ZipOutputStream(new FileOutputStream(partial))) {
            for (int textIndex = 0; textIndex < text.size(); textIndex++) {
                ...
                checkCancelled(control);
                progress.report(Progress.Phase.ZIP, capture.spec.entry, textIndex + 1,
                        text.size(), 0L, total, true);
                ...
                records.add(record);
                progress.report(Progress.Phase.ZIP, capture.spec.entry, textIndex + 1,
                        text.size(), value.length, total, false);
            }
            ...
            for (int remoteIndex = 0; remoteIndex < remote.size(); remoteIndex++) {
                RemoteSpec spec = remote.get(remoteIndex);
                checkCancelled(control);
                progress.report(Progress.Phase.REMOTE, spec.path, remoteIndex + 1,
                        remote.size(), 0L, total, true);
    ...
    static String archiveName(long createdAtMillis) {
        ...
        return "byd-extend-compatibility-" + format.format(new Date(createdAtMillis)) + ".zip";
    }
    ...
    private static void writeChecked(
        ...
        while (offset < value.length) {
            checkCancelled(control);
            int length = Math.min(BUFFER_SIZE, value.length - offset);
    ...
    static final class CancellationException extends IOException {
        CancellationException() { super("export_cancelled"); }
    }

    static final class ExportControl implements LocalAdbClient.OperationCancellation {
        private final AtomicBoolean cancellationRequested = new AtomicBoolean();
        private final AtomicReference<Socket> activeSocket = new AtomicReference<>();
        private final ProgressListener progressListener;

        ExportControl() { this(null); }

        ExportControl(ProgressListener progressListener) {
        ...
        @Override public void registerActiveSocket(Socket socket) {
            ...
            if (cancellationRequested.get() && activeSocket.compareAndSet(socket, null)) {
                try { socket.close(); } catch (IOException ignored) { }
            }
        }

        @Override public void clearActiveSocket(Socket socket) {
            if (socket != null) activeSocket.compareAndSet(socket, null);
        }
    }

    interface ProgressListener { void onProgress(Progress progress); }

    static final class Progress {
        enum Phase { PREPARING, TEXT, REMOTE, ZIP, COMPLETE, CANCELED }
        final Phase phase;
        ...
        Progress(Phase phase, String currentPath, int index, int count,
    ...
    private static final class ProgressReporter {
        ...
        ProgressReporter(ExportControl control) { this.control = control; }

        void report(Progress.Phase phase, String path, int index, int count,
                long bytes, long totalBytes, boolean force) {
            Progress next = new Progress(phase, path, index, count, bytes, totalBytes);
            long now = System.currentTimeMillis();
            boolean transition = last == null || last.phase != next.phase
                    || !last.currentPath.equals(next.currentPath)
                    || last.index != next.index || last.count != next.count;
            if (!force && !transition && now - lastEmit < PROGRESS_INTERVAL_MS) return;
            last = next;
            lastEmit = now;
            if (control != null && control.progressListener != null) {
                try { control.progressListener.onProgress(next); }
        ...
        void cancelled() {
            ...
            report(Progress.Phase.CANCELED,
                    current == null ? "" : current.currentPath,
                    current == null ? 0 : current.index,
                    current == null ? 0 : current.count,
                    current == null ? 0L : current.bytes,
                    current == null ? 0L : current.totalBytes, true);
        }
    }
    ...
    private static final class ProgressOutputStream extends OutputStream {
        ...
        ProgressOutputStream(OutputStream delegate, ExportControl control,
                ProgressReporter progress, String path, int index, int count, long total) {
