...
public final class AdbCoreTest {
    ...
    public void authPolicyAndLaunchStayNarrow() {
        ...
        assertTrue(command.contains("pidof bydextend_helper"));
        assertTrue(command.contains("while [ -n \"$(pidof bydextend_helper"));
        assertTrue(command.contains("helper_stop_timeout; false"));
        assertTrue(command.contains("rm -f /data/local/tmp/bydextend_helper.lock"));
        assertTrue(command.indexOf("helper_stop_timeout; false")
                < command.indexOf("rm -f /data/local/tmp/bydextend_helper.lock"));
        ...
        assertTrue(command.contains("service list 2>/dev/null | grep -q byd_extend_guard"));
    ...
    public void cameraShellKeepsMainLooperAndHasNoQuitSafelyPath() throws Exception {
        ...
        if (!Files.exists(source)) {
            source = Path.of("src/main/java/com/byd/extend/CameraShellMain.java");
        }
    ...
    public void shellRecoveryIsWakeGatedAndCommandIsFixed() {
        ...
        assertArrayEquals(new String[]{
                        ...
                        "-n", "com.byd.extend/com.byd.extend.ShellRecoveryReceiver"
                },
    ...
    public void cameraConfigRejectsUntrustedValues() {
        assertEquals(25, CameraShellProtocol.VERSION);
